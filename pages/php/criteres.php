...
function getFromTotal () {    
    ...
    while ($donnee = $rep->fetch ()) {
        ...
        if ($add) {
            ...
            'labelBot' =>  ($pest == -1  ? ($donnee['MAXPTOT']) : $donnee['MOYPTOT']) . " &micro;g/L <br/> " . $donnee['MAQMOLQ'] . "/" . $donnee['MAXMOLRECH'] . " Q/R",
...
function getFromPest () {
    ...
    while ($donnee = $rep->fetch ()) {
        ...
        if ($add) {
            ...
            'depth' => $donnee['PROFONDEUR_MAXI_POINT'] + $donnee['ALTITUDE'],
            'labelTop' => $donnee['CD_STATION'] . '<br/>' . $donnee['CdMasseDEa'],
            'labelBot' =>  ($donnee['MA_MOY'] . " &micro;g/L <br/> " . $donnee['NBQUANTIF'] . "/" . $donnee['NBANASPERTS1']) . " Q/R" ,
